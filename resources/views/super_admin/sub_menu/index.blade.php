...
@section('content')
    ...
    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div>
                <i class="fas fa-table me-1"></i> {{ __('Sub Menu list') }}
            </div>
            <a class="btn btn-sm btn-primary" href="{{ route('sub.menu.add') }}">
...
@section('script')
    <script>
        $(document).ready(function () {
            const editUrlTemplate = @json(route('sub.menu.edit', ':id'));
            const deleteUrlTemplate = @json(route('sub.menu.delete', ':id'));
            $('#users-table').DataTable({
                processing: true,
                serverSide: true,
                scrollX: true,          // Enables horizontal scrolling
                autoWidth: false,       // Disables DataTables' auto column width guesswork
                ajax: '{{ route("sub.menu.index") }}',
                columns: [
                    {
                        data: 'id',
