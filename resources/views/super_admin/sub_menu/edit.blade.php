...
@section('content')
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('sub.menu.update', $subMenu->id ?? 0) }}">
                @csrf
                        ...
                        <div data-mdb-input-init class="form-outline">
                            ...
                            <select name="menu_id" class="form-select" aria-label="Default select example">
                                ...
                                @foreach($menu ?? [] as $data)
                                    <option
                                        {{ (isset($subMenu->menu_id) && $subMenu->menu_id ==  $data->id) ? 'selected' : '' }} value="{{ $data->id }}">{{ __($data->display_name) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div data-mdb-input-init class="form-outline">
                            ...
                            <input type="text" name="method_name" class="form-control"
                                   value="{{ $subMenu->method_name ?? '' }}"/>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div data-mdb-input-init class="form-outline">
                            <label class="form-label" for="order">{{ __('Order') }}</label>
                            <input type="number" name="order" class="form-control" value="{{ $subMenu->order ?? '' }}"/>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div data-mdb-input-init class="form-outline">
