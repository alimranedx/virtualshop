...
@section('content')
    ...
    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div>
                <i class="fas fa-table me-1"></i> {{ __('Role list') }}
            </div>
            <a class="btn btn-sm btn-primary" href="{{ route('role.add') }}">
...
@section('script')
    <script>
        $(document).ready(function () {
            const editUrlTemplate = @json(route('role.edit', ':id'));
            const deleteUrlTemplate = @json(route('role.delete', ':id'));
